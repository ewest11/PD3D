...
[binarycenter,PSMags,ims]=LoG_3D_LoGthresh(imfile,0,sigma); 
...
d_nt=diff(N); %First derivative

laplacian=diff(N,2); %Second derivative
...
ylim([0 3000]) % Set y axis limits - will vary by image
...
figure;plot(threshrange,[d_nt; 0],'Color',c,'LineWidth',1);
 ylim([-5000 0]) % these limits will change with different images
 xlim([0 0.5])
...
% Dilate and apply Gaussian filter to make centroids look more like the original PSF
dilated=imdilate(maxProjection(newBC),strel('diamond',sigma));
detected=imgaussfilt(dilated,sigma);
detected=detected./max(detected(:));

% Display max projection of detected puncta
